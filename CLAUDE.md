# Claude Code Project Instructions — uds-windows-kotlin

## Project Overview

This is **uds-windows-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`uds_windows`](https://crates.io/crates/uds_windows).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/uds_windows/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/uds_windows>
