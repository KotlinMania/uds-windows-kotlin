...
internal fun lastError(code: Int = -1): WindowsSocketException =
    WindowsSocketException(code, "Windows socket error: $code")
 ...
 */
internal fun cvt(t: Byte): Byte = if (t.isMinusOne()) throw lastError(t.toInt()) else t

internal fun cvt(t: Short): Short = if (t.isMinusOne()) throw lastError(t.toInt()) else t

internal fun cvt(t: Int): Int = if (t.isMinusOne()) throw lastError(t) else t

internal fun cvt(t: Long): Long = if (t.isMinusOne()) throw lastError(t.toInt()) else t
...
class SocketAddr internal constructor(
    ...
    internal companion object {
        fun new(f: (SockaddrUn, IntArray) -> Int): SocketAddr {
            val addr = SockaddrUn()
