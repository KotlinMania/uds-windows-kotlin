...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`uds_windows`](https://crates.io/crates/uds_windows).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `uds_windows` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

UNIX-domain-socket emulation on Windows
...
Original work copyrighted by the upstream `uds_windows` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
